...
for fname in files_to_process:
    ...
    # 1. Remove Gallery menu link
    content = re.sub(r'<li class="nav-item"><a class="nav-link" href="#gallery"><i class="mi mi-gallery nav-icon"></i> Gallery</a></li>', '', content)
    
    # 2. Remove Gallery section
    content = re.sub(r'<section class="section shadow-sm lazyload" id="gallery">.*?</section>', '', content, flags=re.DOTALL)

    # 3. Replace AI bot chat HTML with WhatsApp
    if "<!-- Small Chatbot -->" in content and "<!-- Chatbot -->" in content:
        content = re.sub(r'<!-- Small Chatbot -->.*?<!-- Chatbot -->', whatsapp_html, content, flags=re.DOTALL)
    else:
        # If the tags are missing, just append to body? The tags should be there based on grep
        pass

    # 4. Remove chat CSS and JS
    ...
    content = re.sub(r'<script src="\./assets/js/chatv27\.js"></script>\s*<script>\s*var chatbotApiInput = \{.*?\};\s*</script>', '', content, flags=re.DOTALL)
